...
import PortData from "./ports_data.csv";
import ShipData from "./ship_data.csv";
...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (p === 0) {
          p++;
          //port ------------------
          const response = await axios.get(PortData);
          const parsedData = Papa.parse(response.data, {
            header: true,
            skipEmptyLines: true,
          }).data;
          setPortData(parsedData);
          setLoading(false); // Set loading to false once data is fetched
          const parsedMarkers = parsedData.map((d) => ({
            name: d.port_name,
          ...
          //ship------------------
          const ship = {};
          const response1 = await axios.get(ShipData);
          const parsedData1 = Papa.parse(response1.data, {
            header: true,
            skipEmptyLines: true,
          }).data;
          ...
          console.log("restructured ship data", restructuredShipData);
    ...
    fetchData(); // Call fetchData function once the component mounts
  }, []);
